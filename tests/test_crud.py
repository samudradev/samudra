...
def test_create_golongan_kata():
    data = CreateGolonganKata(id='nama', nama='kata nama', keterangan='kata yang digunakan untuk merujuk kepada benda.')
...
def test_create_golongan_kata_w_value_error():
    with pytest.raises(ValueError):
        CreateGolonganKata(id='namayasa', nama='kata nama',
...
def test_create_konsep():
    ...
    create_golongan_kata(data=CreateGolonganKata(id='NAMA', nama='kata nama',
                                                 keterangan='kata yang digunakan untuk merujuk kepada benda.'))
...
def test_get_minimum_lemma_info():
    from samudra.models import Lemma
    data = AnnotatedText(body=DATA_1)
    create_golongan_kata(data=CreateGolonganKata(id='NAMA', nama='kata nama',
                                                 keterangan='kata yang digunakan untuk merujuk kepada benda.'))
    create_konsep(annotated_text=data, lemma_name='ujian')
    create_konsep(annotated_text=data, lemma_name='ujian')
    lemma = get_lemma_minimum_info(where=(Lemma.nama == 'ujian'), limit=1)[0]
